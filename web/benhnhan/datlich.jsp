...
<section class="container mx-auto px-4 py-20">
    <div class="max-w-3xl mx-auto bg-white p-10 rounded-2xl shadow-lg">
        ...
        <form action="${pageContext.request.contextPath}/LichHenController" method="post" class="space-y-6">
            ...
            <!-- Ca khám (lọc theo ngày) -->
            <div>
                <label class="block font-semibold mb-2 text-gray-700">Chọn ca khám:</label>
                <select id="ca-select" name="idCa" required
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-sky-500">
...
<!-- Script lọc ca khám theo ngày -->
<script>
    // ===== DỮ LIỆU CA KHÁM =====
    const allCaKham = [
        <c:forEach var="ca" items="${listCaKham}" varStatus="loop">
        {
            caId: ${ca.caId},
            tenCa: "<c:out value='${ca.tenCa}'/>",
            gioBatDau: "<c:out value='${ca.gioBatDau}'/>",
            gioKetThuc: "<c:out value='${ca.gioKetThuc}'/>",
            ngay: "<fmt:formatDate value='${ca.ngay}' pattern='yyyy-MM-dd'/>"
        }${!loop.last ? ',' : ''}
        </c:forEach>
    ];

    const ngaySelect = document.getElementById('ngayHen');
    const caSelect   = document.getElementById('ca-select');

    function resetCa(msg = '-- Vui lòng chọn ngày trước --') {
        caSelect.innerHTML = `<option value="none">${msg}</option>`;
    }

    ngaySelect.addEventListener('change', function () {
        const selectedDate = this.value.trim();
        resetCa('-- Vui lòng chọn ca khám --');

        if (!selectedDate) return;

        // Lọc ca theo ngày 
        const filtered = allCaKham.filter(c => c.ngay.trim() === selectedDate);

        if (filtered.length === 0) {
            caSelect.innerHTML = `<option value="none" disabled selected>Không có ca khám trong ngày này</option>`;
            return;
        }

        caSelect.innerHTML = `<option value="">-- Vui lòng chọn ca khám --</option>`;

        filtered.forEach(ca => {
            const opt = document.createElement('option');
            opt.value = ca.caId;
            opt.textContent = ca.tenCa + " (" + ca.gioBatDau + " - " + ca.gioKetThuc + ")";
